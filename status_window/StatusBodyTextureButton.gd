...
func can_drop_data(_position, data):
	...
	if data is Node && data.is_in_group("ITEM") && data.current_item.get_item_type() == GlobalItems.types_of_items.ARMOR:
		can_drop = data.current_item.get_armor_type() == GlobalItems.armor_type.BODY
	return can_drop
...
func load_item(item: Items) -> void:
	...
	current_item = item
	current_equipement[GlobalItems.equipement_type.BODY] = item

func reset_item() -> void:
	texture_normal = null
	current_item = null
	current_equipement[GlobalItems.equipement_type.BODY] = null
